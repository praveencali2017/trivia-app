...
def create_app(test_config=None):
  ...
  def get_questions():
      ...
      offset = (page - 1) *10
      questions = [question.format() for question in Question.query.filter_by(category = category).offset(offset).limit(QUESTIONS_PER_PAGE)]
      data = {
          'questions': questions,
  ...
  def get_category_questions(id: int):
      questions = [question.format() for question in Question.query.filter_by(category = id).limit(QUESTIONS_PER_PAGE)]
      data = {
          'questions': questions,
          'totalQuestions': len(questions),
          'currentCategory': id
